...
from identity_auth_server.core.token.postgres.models import (
    TokenModel,
)
...
class TokenPostgresRepository(TokenRepository):
    ...
    def create_token(self, token: str) -> str:
        """Persist a source app session and return the generated token."""
        db_session = TokenModel(id=uuid4(), token=token)
