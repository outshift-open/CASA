...
class ResourceWatcher:
    ...
    async def subscribe_policy(
        self,
        namespace: Optional[str] = None,
        resource_version: Optional[str] = None,
    ) -> AsyncGenerator[WatchEvent, None]:
        """Subscribe to ZTAPolicy CRD watch events.
        ...
        Yields:
            WatchEvent objects for each change
        """
        subscriber_id = f"policy-{namespace or 'all'}-{id(asyncio.current_task())}"
        queue: asyncio.Queue = asyncio.Queue()
        
        async with self._lock:
            key = f"policy-{namespace or 'all'}"
            if key not in self._policy_subscribers:
                self._policy_subscribers[key] = []
            self._policy_subscribers[key].append(queue)
        
        try:
            logger.info(f"Watch started for ZTAPolicy in namespace={namespace}")
            ...
            # Cleanup
            async with self._lock:
                if key in self._policy_subscribers:
                    self._policy_subscribers[key].remove(queue)
                    if not self._policy_subscribers[key]:
                        del self._policy_subscribers[key]
            logger.info(f"Watch closed for ZTAPolicy in namespace={namespace}")
    ...
    async def notify_policy_event(
        ...
        """Notify all subscribers of a ZTAPolicy change.
        
        Args:
            event_type: Type of change (ADDED, MODIFIED, DELETED)
            policy: The CRD object that changed
            namespace: Namespace of the resource
        """
        async with self._lock:
            # Increment resource version
            ...
            policy.metadata.resource_version = str(current_version)
            
            # Create event
            event = WatchEvent(
                type=event_type,
                object=policy.model_dump(mode="json", exclude_none=True),
            )
            
            # Notify namespace-specific subscribers
            if key in self._policy_subscribers:
                for queue in self._policy_subscribers[key]:
                    try:
                        await queue.put(event)
                    except Exception as e:
                        logger.error(f"Failed to notify subscriber: {e}")
            ...
            if namespace:
                all_key = "policy-all"
                if all_key in self._policy_subscribers:
                    for queue in self._policy_subscribers[all_key]:
                        try:
                            await queue.put(event)
                        except Exception as e:
                            logger.error(f"Failed to notify all subscriber: {e}")
