...
| `agentSafe.docker.image` | Image name within the registry. |
| `agentSafe.tagversion` | Image tag. |
| `agentSafe.mcp_server_url` | MCP server URL the agent calls. |
| `agentCompromised.*` | Same fields as `agentSafe.*`, for the compromised agent. |
| `mcp.serviceName` | K8s Service name for the MCP server. |
...
| `masSafe.llm_host` | LLM FQDN used for eBPF LLM endpoint restriction. |
