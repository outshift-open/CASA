...
#include <pid/maps/allowed_pids.h>
...
static __always_inline u8 pid_matches(pid_data_t *p) {
    ...
    u64 *v = bpf_map_lookup_elem(&allowed_pids, &segment);
    if (!v) {
...
static __always_inline u32 allowed_pid(u64 id) {
    const u32 a_pid = id >> 32;
    // accept all PIDs if debugging EBPF_BPF_PID_FILTER_OFF option is set
