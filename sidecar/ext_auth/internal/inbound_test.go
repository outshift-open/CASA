...
func TestInboundCheck_should_generate_token_for_agent(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"generate token for client app": {appType: identitysdk.CLIENT},
		"generate token for agent app":  {appType: identitysdk.AGENT},
	}
	...
	for tn, tc := range tests {
		t.Run(tn, func(t *testing.T) {
			...
			authSrvClient.EXPECT().
				LoadTokenFromCache(t.Context(), namespace, traceID, host, tc.appType, mock.Anything).
				Return(nil, nil)
			authSrvClient.EXPECT().
			...
			authSrvClient.EXPECT().
				StoreTokenInCache(t.Context(), namespace, traceID, host, tc.appType, accessToken, mock.Anything).
				Return(nil)
