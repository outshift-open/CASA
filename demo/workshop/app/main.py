...
async def chat_with_agent(message: UserMessage, request: Request):
    ...
    # # Get the bearer token from the request state (set by middleware)
    bearer_token = getattr(request.state, "bearer_token", None)
    if not bearer_token:
        return AgentResponse(response="Authentication error: No bearer token found")
