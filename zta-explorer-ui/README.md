...
Modern UI for the ZTA (Zero Trust Architecture) Authorization Server built with React, TypeScript, and shadcn/ui.
...
```bash
# Build the image
docker build -f deployments/docker/Dockerfile.ui -t zta-explorer-ui .

# Run the container
docker run -p 1234:80 zta-explorer-ui
```
...
- `zta-explorer-ui/.env` for the UI configuration
