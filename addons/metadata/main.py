...
# 2. Define the client metadata as a Python dictionary
#    This is the data that will be served as JSON.
CLIENT_METADATA = {
    "client_id": "http://localhost:8081/oauth/client-metadata.json",
...
# 3. Define the route for the metadata endpoint
#    This decorator tells Flask to call the 'get_client_metadata' function
#    when a GET request is made to '/oauth/client-metadata.json'.
@app.route("/oauth/client-metadata.json", methods=["GET"])
def get_client_metadata():
    """
    Exposes the client metadata as a JSON response.
    """
    # The 'jsonify' function from Flask correctly serializes the dictionary
...
if __name__ == "__main__":
    # Running on port 8081 as specified in the 'client_id' field.
